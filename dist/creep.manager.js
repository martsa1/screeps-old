        ...
        for(var name in Game.creeps) {
            ...
            if (creep.memory.role) {
                try {
                    ...
                    console.log(creep.name, 'caught amnesia, former role:', creep.memory.role, ', unsetting role:', err)
    ...
    for(var name in Memory.creeps) {
            ...
            if (creep) {
                if (creep.ticksToLive < 100 && creep.memory.role != 'regenerate') {
                    console.log(creep.name, 'is getting old and is finally entitled to some regeneration!')
...
function spawn_units(current_room) {
    ...
    // console.log('max:',max_build_energy, 'Available:', current_room.energyAvailable)
    ...
    var population_unknown = _.filter(Game.creeps, (creep) => !creep.memory.unit_type)


    // console.log('Needed Workers:', needed_workers, 'Worker pop:', population_workers.length)
    // console.log('Needed Fighters:', needed_fighters, ', Fighter pop:', population_fighters.length)
    ...
    var min_lvl_workers = _.filter(Game.creeps, (creep) => creep.memory.role == Memory.socialStructure[0])
    if (min_lvl_workers.length == Memory.population[Memory.socialStructure[0]]) {
        var worst_creep = _.min(min_lvl_workers, function(worker) { return worker.memory.lvl })
        var worst_level = Number(worst_creep.memory.lvl)
        var max_level = 3
        var next_level = worst_level + 1 > max_level ? max_level : worst_level + 1
...
function workersUnion() {
    ...
    for (var unit_role in Memory.socialStructure) {
        for (var count = 0; count < Memory.population[Memory.socialStructure[unit_role]]; count ++) {
            creeps_assignments.push(Memory.socialStructure[unit_role])
        }
    }

    // Create an ordered list of creeps (hashes are not numbered)
    var creeps_list = _.filter(Game.creeps, (creep) => (creep.memory.unit_type == 'worker'
                                                        && creep.memory.role != 'regenerate'
                                                        && creep.memory.role != 'upgrade'))
        creeps_list = _.sortBy(creeps_list, [function(creep) { return creep.memory.lvl }])
    ...
    if (creeps_assignments.length- 1 - count != 0) {
        console.log('We have a ', creeps_assignments.length- 1 - count , 'worker deficit!')
    }
